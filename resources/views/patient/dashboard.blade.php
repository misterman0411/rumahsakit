...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="stat-card">
        ...
        <p class="text-3xl font-bold text-gray-900">{{ $stats['total_prescriptions'] }}</p>
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <a href="{{ route('patient.appointments.book') }}" class="bg-white/20 hover:bg-white/30 backdrop-blur rounded-xl p-4 text-center transition-all hover:scale-105">
                    <svg class="w-8 h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                    </svg>
                    <span class="text-sm font-medium">Booking Baru</span>
                </a>
                <a href="{{ route('patient.medical-records') }}" class="bg-white/20 hover:bg-white/30 backdrop-blur rounded-xl p-4 text-center transition-all hover:scale-105">
                    <svg class="w-8 h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
                    <span class="text-sm font-medium">Rekam Medis</span>
                </a>
                <a href="{{ route('patient.invoices') }}" class="bg-white/20 hover:bg-white/30 backdrop-blur rounded-xl p-4 text-center transition-all hover:scale-105">
                    <svg class="w-8 h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
                    </svg>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <a href="{{ route('patient.appointments') }}" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium">Lihat Semua →</a>
            </div>
            <div class="p-6">
                        ...
                        @foreach($upcomingAppointments as $appointment)
                            <div class="flex items-center p-4 bg-gray-50 rounded-xl hover:bg-indigo-50 transition-colors">
                                <div class="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex flex-col items-center justify-center text-white mr-4">
                                    ...
                                    <span class="text-lg font-bold">{{ \Carbon\Carbon::parse($appointment->tanggal_janji)->format('d') }}</span>
                                </div>
                                <div class="flex-1">
                                    <p class="font-semibold text-gray-900">{{ $appointment->dokter->user->nama ?? 'Dokter' }}</p>
                                    <p class="text-sm text-gray-500">{{ $appointment->departemen->nama ?? 'Poliklinik' }} • {{ \Carbon\Carbon::parse($appointment->tanggal_janji)->format('H:i') }}</p>
                                </div>
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <a href="{{ route('patient.appointments.book') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm font-semibold text-gray-900">{{ $patient->no_rekam_medis }}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm font-semibold text-gray-900">{{ $patient->golongan_darah ?? '-' }}</span>
                    </div>
                </div>
            </div>
        @else
                    ...
                    <div class="space-y-3">
                        @foreach($activePrescriptions as $prescription)
                            <div class="p-3 bg-gray-50 rounded-lg">
                                <p class="text-sm font-semibold text-gray-900">{{ $prescription->dokter->user->nama ?? 'Dokter' }}</p>
                                <p class="text-xs text-gray-500">{{ $prescription->created_at->format('d M Y') }}</p>
                            </div>
                        @endforeach
                    </div>
                @else
