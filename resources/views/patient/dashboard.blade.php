...
@section('subtitle', 'Ringkasan informasi kesehatan Anda')
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-8 text-white shadow-xl shadow-indigo-200">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <a href="{{ route('patient.appointments.book') }}" class="group bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/10 rounded-xl p-4 text-center transition-all hover:scale-105">
                ...
                <a href="{{ route('patient.medical-records') }}" class="group bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/10 rounded-xl p-4 text-center transition-all hover:scale-105">
                    <div class="w-10 h-10 mx-auto mb-3 bg-white/20 rounded-lg flex items-center justify-center group-hover:rotate-6 transition-transform">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('patient.invoices') }}" class="group bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/10 rounded-xl p-4 text-center transition-all hover:scale-105">
                    <div class="w-10 h-10 mx-auto mb-3 bg-white/20 rounded-lg flex items-center justify-center group-hover:rotate-6 transition-transform">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
                        </svg>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                ...
                <a href="{{ route('patient.appointments') }}" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium flex items-center gap-1">
                    Lihat Semua 
            ...
            <div class="p-6">
                        ...
                        @foreach($upcomingAppointments as $appointment)
                            <div class="flex items-center p-4 bg-white border border-gray-100 rounded-xl hover:shadow-md hover:border-indigo-100 transition-all">
                                <div class="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex flex-col items-center justify-center text-white mr-5 shadow-sm">
                                    <span class="text-xs font-medium uppercase tracking-wider">{{ \Carbon\Carbon::parse($appointment->tanggal_janji)->format('M') }}</span>
                                    <span class="text-2xl font-bold">{{ \Carbon\Carbon::parse($appointment->tanggal_janji)->format('d') }}</span>
                                </div>
                                <div class="flex-1">
                                    <h4 class="font-bold text-gray-900 text-lg">{{ $appointment->dokter->user->nama ?? 'Dokter' }}</h4>
                                    <div class="flex items-center text-gray-500 mt-1 space-x-3 text-sm">
                                        <span class="flex items-center gap-1">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                                            </svg>
                                            {{ $appointment->departemen->nama ?? 'Poliklinik' }}
                                        </span>
                                        <span class="flex items-center gap-1">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                            {{ \Carbon\Carbon::parse($appointment->tanggal_janji)->format('H:i') }}
                                        </span>
                                    </div>
                                </div>
                                <span class="px-4 py-1.5 text-xs font-bold rounded-full 
                                    @if($appointment->status === 'dikonfirmasi') bg-green-100 text-green-700
                ...
                @else
                    <div class="text-center py-10 bg-gray-50 rounded-xl border-dashed border-2 border-gray-200">
                        ...
                        <a href="{{ route('patient.appointments.book') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">
    ...
    <div class="space-y-8">
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        <div class="flex items-center justify-between pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                            ...
                            <span class="text-sm font-bold text-indigo-700 bg-indigo-50 px-2 py-1 rounded">{{ $patient->no_rekam_medis }}</span>
                        </div>
                        <div class="flex items-center justify-between pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                            <span class="text-sm text-gray-500">Nama Lengkap</span>
                            <span class="text-sm font-semibold text-gray-900">{{ $patient->nama }}</span>
                        </div>
                        <div class="flex items-center justify-between pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                            <span class="text-sm text-gray-500">Tanggal Lahir</span>
                            <span class="text-sm font-semibold text-gray-900">{{ $patient->tanggal_lahir ? $patient->tanggal_lahir->format('d M Y') : '-' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-gray-500">Golongan Darah</span>
                            <span class="text-sm font-bold text-red-600 bg-red-50 px-3 py-1 rounded-full">{{ $patient->golongan_darah ?? '-' }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                ...
                <a href="{{ route('patient.prescriptions') }}" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium">Lihat →</a>
            </div>
            <div class="p-6">
                        ...
                        @foreach($activePrescriptions as $prescription)
                            <div class="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:bg-purple-50 hover:border-purple-100 transition-colors">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">{{ $prescription->created_at->format('d M Y') }}</span>
                                    <span class="w-2 h-2 rounded-full bg-green-500"></span>
                                </div>
                                <p class="text-sm font-bold text-gray-900 mb-1">{{ $prescription->dokter->user->nama ?? 'Dokter' }}</p>
                                <p class="text-xs text-gray-500 truncate">No. Resep: #{{ $prescription->id }}</p>
                            </div>
                        @endforeach
                    </div>
                @else
