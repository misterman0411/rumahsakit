                ...
                <div class="p-6 space-y-6">
                        ...
                        <select name="dokter_id" id="dokter_id" required
                            ...
                            @foreach($doctors as $doctor)
                                <option value="{{ $doctor->id }}" 
                                    {{ (old('dokter_id') == $doctor->id || (auth()->user()->dokter && auth()->user()->dokter->id == $doctor->id)) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="jenis_tes_id" id="jenis_tes_id" required
                            ...
                            @foreach($testTypes as $testType)
                                <option value="{{ $testType->id }}" {{ old('jenis_tes_id') == $testType->id ? 'selected' : '' }}>
                                    {{ $testType->nama }} - Rp {{ number_format($testType->harga, 0, ',', '.') }}
                                </option>
                            @endforeach
                        </select>
                        @error('jenis_tes_id')
                    ...
                    <div>
                        ...
                        <textarea name="catatan_klinis" id="catatan_klinis" rows="4"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            placeholder="Riwayat klinis, indikasi pemeriksaan, area yang diperiksa, dll">{{ old('catatan_klinis') }}</textarea>
