...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="flex rounded-lg overflow-hidden border border-gray-300">
                ...
                <a href="{{ route('management.staff-performance', ['period' => $key]) }}" 
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ count($doctorPerformance) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ count($labTechPerformance) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ count($radiologistPerformance) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl shadow-sm p-6 text-white">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ count($doctorPerformance) + count($labTechPerformance) + count($radiologistPerformance) }}</p>
                </div>
            </div>
        </div>
    </div>

    <!-- Recent Staff Activities (Log) -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h2 class="text-lg font-bold text-gray-800 mb-4">Recent Doctor Activities ({{ ucfirst($period) }})</h2>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($recentStaffActivities as $activity)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $activity->updated_at->format('d M Y H:i') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-bold">
                            {{ $activity->dokter->user->nama ?? 'Unknown' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $activity->pasien->nama ?? '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $activity->departemen->nama ?? '-' }}
                        </td>
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-4 whitespace-nowrap text-center text-gray-500">
                            No recent activities found (completed appointments).
                        </td>
                    </tr>
                    @endforelse
                </tbody>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                                    <span class="text-blue-600 font-bold text-sm">{{ strtoupper(substr($doctor->nama, 0, 2)) }}</span>
                                </div>
                                <span class="font-medium">{{ $doctor->nama }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-gray-600">{{ $doctor->departemen ?? '-' }}</td>
                        <td class="px-4 py-3 text-center font-semibold">{{ $doctor->total_appointments }}</td>
                        <td class="px-4 py-3 text-center font-semibold text-green-600">{{ $doctor->completed_appointments }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-1 rounded-full text-xs font-bold {{ $badgeBgClass }} {{ $badgeTextClass }}">
                                {{ $rate }}%
                            </span>
                        </td>
                        <td class="px-4 py-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                @forelse($labTechPerformance as $tech)
                <div class="border border-gray-200 rounded-lg p-4">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-purple-100 rounded-full flex items-center justify-center">
                                <span class="text-purple-600 font-bold text-sm">{{ strtoupper(substr($tech->nama, 0, 2)) }}</span>
                            </div>
                            <span class="font-medium">{{ $tech->nama }}</span>
                        </div>
                        <span class="text-2xl font-bold text-purple-600">{{ $tech->total_tests }}</span>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                @forelse($radiologistPerformance as $rad)
                <div class="border border-gray-200 rounded-lg p-4">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
                                <span class="text-green-600 font-bold text-sm">{{ strtoupper(substr($rad->nama, 0, 2)) }}</span>
                            </div>
                            <span class="font-medium">{{ $rad->nama }}</span>
                        </div>
                        <span class="text-2xl font-bold text-green-600">{{ $rad->total_exams }}</span>
                    </div>
