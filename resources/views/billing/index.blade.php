    ...
    <div class="p-6">
        ...
        <form method="GET" action="{{ route('billing.index') }}" class="mb-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nomor invoice atau nama pasien..." class="px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route('billing.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
                        Reset
                    </a>
                    @endif
                </div>
...
<div id="multiplePaymentModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-lg bg-white">
        ...
        <!-- Manual Payment Form -->
        <form id="manualPaymentForm" action="{{ route('billing.payment-multiple') }}" method="POST">
            @csrf
        ...
        <form id="midtransPaymentForm" action="{{ route('billing.payment-multiple-midtrans') }}" method="POST" class="hidden">
            @csrf
...
function openMultiplePaymentModal() {
    ...
    window.location.href = '{{ route("billing.payment-multiple.show") }}?invoice_ids=' + encodeURIComponent(JSON.stringify(ids));
}
