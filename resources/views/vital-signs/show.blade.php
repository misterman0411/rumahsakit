    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="text-gray-600 mt-2">Recorded on {{ $vitalSign->recorded_at->format('d F Y, H:i') }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-semibold text-gray-900">{{ $vitalSign->pasien->nama }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">MRN</p>
                            <p class="font-semibold text-gray-900">{{ $vitalSign->pasien->no_rekam_medis }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Age</p>
                            <p class="font-semibold text-gray-900">{{ $vitalSign->pasien->tanggal_lahir->age }} years</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Gender</p>
                            <p class="font-semibold text-gray-900">{{ ucfirst($vitalSign->pasien->gender) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gradient-to-br from-red-50 to-red-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-red-900">{{ $vitalSign->blood_pressure ?? '-' }}</p>
                        ...
                        <div class="bg-gradient-to-br from-orange-50 to-orange-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-orange-900">{{ $vitalSign->temperature ?? '-' }}</p>
                        ...
                        <div class="bg-gradient-to-br from-pink-50 to-pink-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-pink-900">{{ $vitalSign->heart_rate ?? '-' }}</p>
                        ...
                        <div class="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-blue-900">{{ $vitalSign->respiratory_rate ?? '-' }}</p>
                        ...
                        <div class="bg-gradient-to-br from-cyan-50 to-cyan-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-cyan-900">{{ $vitalSign->oxygen_saturation ?? '-' }}</p>
                        ...
                        <div class="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-green-900">{{ $vitalSign->weight ?? '-' }}</p>
                        ...
                        <div class="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-purple-900">{{ $vitalSign->height ?? '-' }}</p>
                        ...
                        <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-indigo-900">{{ number_format($vitalSign->bmi, 1) }}</p>
                ...
                <!-- Notes -->
                @if($vitalSign->catatan)
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h2 class="text-xl font-bold text-gray-800 mb-4">Notes</h2>
                    <p class="text-gray-700 whitespace-pre-wrap">{{ $vitalSign->catatan }}</p>
                </div>
                @endif
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        @foreach($recentVitals as $recent)
                        <div class="border-l-4 border-indigo-500 pl-4 py-2 bg-gray-50 rounded">
                            <p class="text-sm font-semibold text-gray-900">{{ $recent->recorded_at->format('d M Y, H:i') }}</p>
                            <div class="grid grid-cols-4 gap-2 mt-2 text-xs text-gray-600">
                                <div><span class="font-medium">BP:</span> {{ $recent->blood_pressure ?? '-' }}</div>
                                <div><span class="font-medium">Temp:</span> {{ $recent->temperature ?? '-' }}°C</div>
                                <div><span class="font-medium">HR:</span> {{ $recent->heart_rate ?? '-' }} bpm</div>
                                <div><span class="font-medium">SpO2:</span> {{ $recent->oxygen_saturation ?? '-' }}%</div>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="font-semibold text-gray-900">{{ $vitalSign->recorded_at->format('d F Y') }}</p>
                            <p class="text-sm text-gray-700">{{ $vitalSign->recorded_at->format('H:i') }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Recorded By</p>
                            <p class="font-semibold text-gray-900">{{ $vitalSign->recordedBy->nama ?? 'N/A' }}</p>
                        </div>
                        @if($vitalSign->medicalRecord)
                        <div>
                            <p class="text-sm text-gray-600">Medical Record</p>
                            <a href="{{ route('medical-records.show', $vitalSign->medicalRecord) }}" 
                                class="font-semibold text-indigo-600 hover:text-indigo-800">
                                {{ $vitalSign->medicalRecord->record_number }}
                            </a>
                        </div>
                        @endif
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('patients.show', $vitalSign->pasien) }}" 
