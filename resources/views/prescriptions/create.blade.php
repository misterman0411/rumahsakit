                            ...
                            <div class="medication-item border border-gray-200 rounded-lg p-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="col-span-2">
                                        ...
                                        <select name="items[0][medication_id]" required
                                            ...
                                            @foreach($medications as $medication)
                                                <option value="{{ $medication->id }}" data-price="{{ $medication->price }}">
                                                    {{ $medication->nama }} - Rp {{ number_format($medication->price, 0, ',', '.') }} (Stok: {{ $medication->stock }})
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="items[0][dosage]" required placeholder="Contoh: 500mg"
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-2">Frekuensi</label>
                                        <input type="text" name="items[0][frequency]" required placeholder="Contoh: 3x sehari"
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-2">Durasi</label>
                                        <input type="text" name="items[0][duration]" required placeholder="Contoh: 7 hari"
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div class="col-span-2">
                                        <label class="block text-sm font-semibold text-gray-700 mb-2">Instruksi</label>
                                        <textarea name="items[0][instructions]" rows="2"
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
    ...
    const newMedication = `
        <div class="medication-item border border-gray-200 rounded-lg p-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Obat</label>
                    <select name="items[${medicationIndex}][medication_id]" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        <option value="">-- Pilih Obat --</option>
                        ${medicationsData.map(med => `
                            <option value="${med.id}" data-price="${med.price}">
                                ${med.name} - Rp ${new Intl.NumberFormat('id-ID').format(med.price)} (Stok: ${med.stock})
                            </option>
                        `).join('')}
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Jumlah</label>
                    <input type="number" name="items[${medicationIndex}][quantity]" min="1" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Dosis</label>
                    <input type="text" name="items[${medicationIndex}][dosage]" required placeholder="Contoh: 500mg"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Frekuensi</label>
                    <input type="text" name="items[${medicationIndex}][frequency]" required placeholder="Contoh: 3x sehari"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Durasi</label>
                    <input type="text" name="items[${medicationIndex}][duration]" required placeholder="Contoh: 7 hari"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                </div>
                <div class="col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Instruksi</label>
                    <textarea name="items[${medicationIndex}][instructions]" rows="2"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                        placeholder="Contoh: Diminum sesudah makan"></textarea>
                </div>
            </div>
